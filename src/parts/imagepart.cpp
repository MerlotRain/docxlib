...
ImagePart::ImagePart(const PackURI &partName, const QString &contentType, const QByteArray &blob,
                     Docx::Image *image, const QByteArray &hash)
    : Part(partName, contentType, blob)
{
    ...
    if (hash.isEmpty()) m_hash = getFileHash(partName);
    else
        m_hash = hash;
}
...
{
    if (m_partName.isEmpty()) return QString("image.%1").arg(m_image->ext());
...
ImagePart *ImagePart::load(const PackURI &partName, const QString &contentType,
                           const QByteArray &blob, Image *image)
{
...
Image *ImagePart::image() const { return m_image; }

QByteArray ImagePart::hash() const { return m_hash; }
...
QByteArray ImagePart::blob() const { return m_image->blob(); }
