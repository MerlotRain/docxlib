...
{
    if (m_overrides.contains(partname.fullURI())) return m_overrides.value(partname.fullURI());
    if (m_defaults.contains(partname.ext())) return m_defaults.value(partname.ext());
