...
PackageWriter::PackageWriter(const QString &pkgFile) { m_writer = new PhysPkgWriter(pkgFile); }

PackageWriter::PackageWriter(const QString &pkgFile, const Relationships *pkgRels,
...
void PackageWriter::writeParts(const QList<Part *> &parts)
{
    ...
    while (iter.hasNext())
    {
        ...
        if (rles->count() > 0) m_writer->write(p->partName().relsUriStr(), rles->blob());
    }
}

PackageWriter::~PackageWriter() { delete m_writer; }
