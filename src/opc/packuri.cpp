...
QString PackURI::fullURI() const { return QString("/").append(*this); }
...
int PackURI::idx() const
{
    ...
    {
        if (namePart.at(i).isNumber()) sl.insert(0, namePart.at(i));
        else
            break;
    }
...
QString PackURI::memberName() const { return this->mid(1); }

QString PackURI::ext() const { return this->split('.').last(); }


QString PackURI::relativeRef(const QString &baseURI)
{
    QString str;
    if (baseURI == QStringLiteral("/")) str = this->mid(1);
    else
    {
