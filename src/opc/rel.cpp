...
Relationship::Relationship(const QString &rId, const QString &reltype, const QString &targetRef,
                           Part *target, const QString &baseURI, bool external)
    : m_rId(rId), m_targetRef(targetRef), m_reltype(reltype), m_baseURI(baseURI),
      m_isexternal(external), m_target(target)
{
}

QString Relationship::targetRef() const { return m_targetRef; }

Relationship::~Relationship() { delete m_target; }


Relationships::Relationships(const QString &baseURI) : m_baseURI(baseURI) {}

Relationship *Relationships::addRelationship(const QString &reltype, const QString &targetRef,
                                             Part *target, const QString &rId, bool external)
{
    Relationship *rel = new Relationship(rId, reltype, targetRef, target, m_baseURI, external);
    m_rels[rId] = rel;
    if (!external) m_targetPartsByrId[rId] = target;
    return rel;
}
...
Part *Relationships::partWithReltype(const QString &reltype)
{
    ...
    {
        if (s->relType() == reltype) return s->target();
    }
    return nullptr;
}

int Relationships::count() const { return m_rels.count(); }

QMap<QString, Relationship *> Relationships::rels() const { return m_rels; }
...
Relationship *Relationships::getOrAddPart(const QString &reltype, Part *target,
...
Relationship *Relationships::getMatchingExt(const QString &reltype, const QString &targetref)
{
    for (Relationship *rel: m_rels.values())
    {
        if (reltype != rel->relType()) return nullptr;
        if (!rel->isExternal()) return nullptr;
        if (rel->targetRef() != targetref) return nullptr;
...
QString Relationships::nextrId()
{
    ...
    for (int i = 1; i < size; i++)
    {
        ...
        if (!keys.contains(rId)) return rId;
    }
    return QString();
}
